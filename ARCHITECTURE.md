...
GunDB is an adapter, not a second authority plane. In-graph Gun data is the native ACL. A Check object is a Gun-native feed item, a held claim, or a later opaque post/room id (same Check). Mesh see grants HAM-merge under `s3rch/acl/…` and do not fork items or users. A permalink / RSS3 / RSS / issuer HTTP URL is a leaf, not a node. A handoff hint is untrusted and cannot mint. A Social Light hop may factor Check and cannot mint. Destination Check (including `delegate`) issues the grant. `crates/sociacl-gun` is the reference implementation. s3r.ch copies [docs/s3rch-check.d.ts](docs/s3rch-check.d.ts) and runs light Check in the browser. It does not import this crate. See [docs/s3rch-check.md](docs/s3rch-check.md) and [docs/gun.md](docs/gun.md). AImmune / Brewnix IR copies [docs/aimmune-ir-check.d.ts](docs/aimmune-ir-check.d.ts) and runs light Check + `delegate` in the browser. It does not import this crate. See [docs/aimmune-ir-check.md](docs/aimmune-ir-check.md).
