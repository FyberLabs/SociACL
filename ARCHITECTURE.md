...
[FyberLabs/socialight](https://github.com/FyberLabs/socialight) owns badge, station, and hop delivery. This repo owns the hop frame (`SLHF`) and verb evaluation. `crates/social-light` is a local lab so that frame can be tested here; it is not the product sibling and it is not a hosted service. FlexModule (2014) is ancestor badge hardware, not a friend edge. See [docs/social-light.md](docs/social-light.md).
